...
// .env.local loads first so it can take precedence over .env (dotenv does not overwrite already-set vars)
...
if (!supabaseUrl) {
  console.error("❌ Error: EXPO_PUBLIC_SUPABASE_URL is required for seeding.");
  console.error(
    "Please add EXPO_PUBLIC_SUPABASE_URL to your .env or .env.local file.",
  );
  process.exit(1);
}
